<template>
    <v-main class="container">
        ...
        <v-dialog v-model=" dialogAddEdit " persistent max-width="600px" style="z-index: 1000;">
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <v-btn color="blue" text @click=" dialogUpdate = true " v-show=" updateButton ">Save</v-btn>
                    <v-btn color="blue" text @click=" dialogAdd = true " v-show=" addShow ">Save</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>
        ...
        <v-dialog v-model=" dialogAdd " persistent max-width="600px" style="z-index: 1000;">
                <v-card>
                    <v-card-title class="text-h5 red lighten-2">
                        <v-toolbar color="red" dark >Are You Sure add This Pegawai?</v-toolbar>
                    </v-card-title>
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="red" text @click=" dialogAdd = false ">NO</v-btn>
                        <v-btn color="red" text @click=" addProcess" >YES</v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>
            ...
            <v-dialog v-model=" dialogUpdate " persistent max-width="600px" style="z-index: 1000;">
                <v-card>
                    <v-card-title class="text-h5 red lighten-2">
                        <v-toolbar color="red" dark >Are You Sure To edit  This Pegawai?</v-toolbar>
                    </v-card-title>
                    <v-card-actions>
                        <v-spacer></v-spacer>
            ...
            if (this.formInstruktur.namaistruktur == '' || this.formInstruktur.telponstruktur == '' || this.formInstruktur.alamatinstruktur == '' || this.formInstruktur.tanggal_lahir == '') {
                ...
                const up = await axios.post(store.state.domainName + 'instruktur', addData)
                    .then(async () => {
                        const data = await axios.get(store.state.domainName + 'instruktur')
                        store.commit('setDataInstruktur', data.data)
                        toastr.success('instruktur Added', setTimeout(() => {
                            this.dialogAddEdit = false
                            this.clearFuntion()
                            router.go()
                        }, 1000))
            ...
            if (this.formInstruktur.namaistruktur == '' || this.formInstruktur.telponstruktur == '' || this.formInstruktur.alamatinstruktur == '' || this.formInstruktur.tanggal_lahir == '') {
                ...
                const up = await axios.put(store.state.domainName + 'instruktur/' + this.id, addData)
                    .then(async () => {
                        const data = await axios.get(store.state.domainName + 'instruktur')
                        store.commit('setDataInstruktur', data.data)
                        toastr.success('instruktur Updated', setTimeout(() => {
                            this.dialogAddEdit = false
                            router.go()
                        }, 1000))
                    }).catch((err) => {
