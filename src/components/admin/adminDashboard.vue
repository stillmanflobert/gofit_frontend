<template>
    <v-main class="container">
        ...
        <v-row>
            <v-col v-for="promo in filteredList" :key="promo.ID_PROMO">
        ...
        <v-dialog v-model="dialog2" persistent max-width="600px" style="z-index: 1000;">
            <v-card>
                ...
                <v-card-actions>
                    ...
                    <v-btn color="red" text @click="dialogUpdate = true" v-show="updateButton">Save</v-btn>
                    <v-btn color="red" text @click="dialogAdd = true" v-show="addShow">Save</v-btn>
                </v-card-actions>
            </v-card>
        </v-dialog>


        <v-dialog v-model=" dialogAdd " persistent max-width="600px" style="z-index: 1000;">
                <v-card>
                    <v-card-title class="text-h5 red lighten-2">
                        <v-toolbar color="red" dark >Are You Sure add This Promo?</v-toolbar>
                    </v-card-title>
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="red" text @click=" dialogAdd = false ">NO</v-btn>
                        <v-btn color="red" text @click=" addPromoProcess" >YES</v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>

            <v-dialog v-model=" dialogUpdate " persistent max-width="600px" style="z-index: 1000;">
                <v-card>
                    <v-card-title class="text-h5 red lighten-2">
                        <v-toolbar color="red" dark >Are You Sure To edit  This Promo?</v-toolbar>
                    </v-card-title>
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="red" text @click=" dialogUpdate = false ">NO</v-btn>
                        <v-btn color="red" text @click=" updateProcess" >YES</v-btn>
                    </v-card-actions>
                </v-card>
            </v-dialog>
    </v-main>
...
export default {
    ...
    methods: {
        ...
        updateProcess() {
            if(this.formUpdate.namapromo == '' || this.formUpdate.dekripsi == '' || this.formUpdate.waktumulai == '' || this.formUpdate.waktuselesai == ''){
    ...
    computed: {
        filteredList() {
            return this.promos.filter(promo => {
                return promo.NAMA_PROMO.toLowerCase().includes(this.search.toLowerCase())
            })
        }
    }
}
